  ...
  QWidget(parent)
{
  ...
  m_currDSlice = m_currWSlice = m_currHSlice = m_currSlice = 0;
...
float RemapImage::zoom() { return m_zoom; }
void
RemapImage::setOffset(int ox, int oy)
{
  m_offsetX = ox;
  m_offsetY = oy;
}
...
int RemapImage::depthSliceNumber() { return m_currDSlice; }
int RemapImage::widthSliceNumber() { return m_currWSlice; }
int RemapImage::heightSliceNumber(){ return m_currHSlice; }
...
void
RemapImage::sliceChangedLowres(int slc)
{
  if (m_sliceType == DSlice)
    m_currDSlice = m_currSlice = slc;
  else if (m_sliceType == WSlice)
...
void
RemapImage::setHiresImage(QImage img, int wd, int ht, int level)
{
  m_lowresImage = false;
  m_level = level;
  setImage(img, wd, ht);
...
void
RemapImage::setLowresImage(QImage img, int wd, int ht, int level)
{
  m_lowresImage = true;
  m_level = level;
  setImage(img, wd, ht);
...
RemapImage::updateStatusText()
{
  ...
  if (gb > 1.0)
    gbtext = QString(" Size(%1 Gb)").arg(gb);
  else
    {
      gb *= 1024;
      gbtext = QString(" Size(%1 Mb)").arg(gb);
    }
  txt += gbtext;
